/**
 * Client-Side AI Validation Layer Engine
 * Provides instant browser-side validation, anomaly detection, and human-readable error reporting
 * for AI-generated JSON payloads, allowing live manual corrections before database import.
 */
  ...
  if (!timetableInput || typeof timetableInput !== 'object') {
    return {
      ...
      issues: [{
        id: 'err_root',
        field: 'timetable',
        message: 'Invalid AI response payload: Expected a valid JSON object.',
        severity: 'ERROR',
        context: {},
      }],
      sanitizedData: null,
    ...
    lectures.forEach((lec, idx) => {
      ...
      if (!startTime || !isValidTime(startTime)) {
        issues.push({
          ...
          field: 'startTime',
          message: `Invalid start time "${rawStart}" for ${subject} on ${day}. Expected HH:MM format.`,
          severity: 'ERROR',
          context: itemContext,
        });
      }
      ...
      if (!endTime || !isValidTime(endTime)) {
        issues.push({
          ...
          field: 'endTime',
          message: `Invalid end time "${rawEnd}" for ${subject} on ${day}. Expected HH:MM format.`,
          severity: 'ERROR',
          context: itemContext,
        });
      }
